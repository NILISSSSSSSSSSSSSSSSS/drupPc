         ...
         <div class="searchTable">
           <!-- 搜索框 -->
                ...
                <el-table ref="multipleDrugerTable" :data="druggerList" border tooltip-effect="dark"
                  @row-click="selectRow" style="width: 100%" @selection-change="selectDruger">
                   <el-table-column type="index" :index="indexMethod" width="50" fixed>
                    ...
                    <el-table-column label="工作人员" :label-width="drugLabelWidth" sortable="custom">
                        <template slot-scope="scope">
                            {{ scope.row.workName}}
                        </template>
                        </el-table-column>
               ...
               <div class="block searchArea" v-if="workList.length!==0">
                 ...
                 <el-select  v-model="currentSelectWorkPerson" value-key="PhoneNumber" placeholder="请选择工作人员">
                    <el-option
                      ...
                      <span style="float: right; color: #8492a6; font-size: 13px">电话号码：{{ item.Count }}</span>
                    </el-option>
                </el-select>
                  <el-button type="primary" @click="addPerson()">调整</el-button>
                </div>
            </div>
            </div>
  ...
  export default {
    ...
    ,methods: {
      ...
      countyChange(type,index){
       ...
       //调整工作人员
      addPerson() {
        if (!this.currentSelectWorkPerson.Name) {
        ...
        if (this.drugerSelection.length === 0 ) {
         ...
         console.log("工作电话:"+_this.currentSelectWorkPerson.PhoneNumber+"选中id:"+newDrugCradId)
        ...
        var paramData={
                ...
                WorkerPhoneNumber: _this.currentSelectWorkPerson.PhoneNumber,
            };

           var newArr= _this.drugerSelection.map(ele => ele.Name)//工作人员名字列表
            this.$confirm(`将把 ${newArr.join(",")} 调整给 ${ _this.currentSelectWorkPerson.Name}，是否继续？`, '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning',
                    center: true
                    }).then(() => {
                         _this.$http.post(_this.httpUrl+"/backstage/index/newtransferstaff",paramData)
                        .then(res => {
                            if(res.data.errno === 0){
                                _this.$notify({
                                        duration:1500,
